...
@Client.on_message(command(["pm", "antipm"]) & filters.me)
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await Groot.set_permit_message(Groot.PMPERMIT_MESSAGE)
        await message.edit("**Anti_PM message set to default**.")
        return
    await Groot.set_permit_message(f"`{arg}`")
...
async def setblkmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await Groot.set_block_message(Groot.BLOCKED)
        await message.edit("**Block message set to default**.")
        return
    await Groot.set_block_message(f"`{arg}`")
...
async def allow(client, message):
    ...
    pmpermit, pm_message, limit, block_message = await Groot.get_pm_settings()
    await Groot.allow_user(chat_id)
...
`.pm [on or off]` - Aᴄᴛɪᴠᴀᴛᴇ Oʀ Dᴇᴀᴄᴛɪᴠᴀᴛᴇ Aɴᴛɪ-Pᴍ
