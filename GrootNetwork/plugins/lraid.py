...
async def replyramd(client: Client, message: Message):
    ...
    if userz.id == mee.id:
        await Groot.edit("`Jaa Na Tharki Kahe Dimag Kha rha? Khudpe Raid kyu laga rha?`")
        return
    if await loveub_info(userz.id):
        await Groot.edit("`Who So Noob? LoveRaid Already Activated on that User:/`")
        return
    ...
    if not chat_dict:
        ...
        return
    await Groot.edit("`Activating LoveRaid....!`")
...
async def dreplyramd(client: Client, message: Message):
    ...
    failed = 0
    if not user:
        await Groot.edit("`Reply To User Or Mention To Deactivate LoveRaid`")
        return
    try:
        userz = await client.get_users(user)
    except:
    ...
    mee= await client.get_me()
    if userz.id == mee.id:
        await Groot.edit("`Soja Tharki`")
        return
    if not await loveub_info(userz.id):
        await Groot.edit("`When I LoveRaid Activated? On That User?:/`")
        return
    ...
    if not chat_dict:
        Groot.edit("`You Have No Chats! So Sad`")
        return
    await Groot.edit("`De-Activating LoveRaid Raid....!`")
