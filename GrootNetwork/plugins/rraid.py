...
from GrootNetwork.config import SUDO_USERS
...
from pyrogram.types import (
    ...
    Message)
from GrootNetwork.utilities.mongo import grootub_info, rgroot, rungroot
...
async def replyramd(client: Client, message: Message):
    ...
    if userz.id == mee.id:
        await Groot.edit("`Jaa Na Lawde Kahe Dimag Kha rha? Khudpe Raid kyu laga rha?`")
        return
    if await grootub_info(userz.id):
        await Groot.edit("`Who So Noob? Reply Raid Already Activated on that User:/`")
        return
    if int(userz.id) in SUDO_USERS:
        await Groot.edit("Abe Lawde that guy part of my devs.")
        return
    await Groot.edit("`Please, Wait Fectching Using Details!`")
    chat_dict = await iter_chats(client)
    chat_len = len(chat_dict)
    if not chat_dict:
        Groot.edit("`You Have No Chats! So Sad`")
        return
    await Groot.edit("`Activating Replyraid....!`")
    await rgroot(userz.id, reason)
    gbanned = f"Reply Raid has Been Activated On {userz.first_name}"
    await Groot.edit(gbanned)
...
async def dreplyramd(client: Client, message: Message):
    ...
    failed = 0
    if not user:
        await Groot.edit("`Reply To User Or Mention To Deactivate Replyraid`")
        return
    try:
        userz = await client.get_users(user)
    except:
        await Groot.edit(f"`404 : User Doesn't Exists!`")
        return
    mee= await client.get_me()
    if userz.id == mee.id:
        await Groot.edit("`Soja Lomde`")
        return
    if not await kaalub_info(userz.id):
        await Groot.edit("`When I Replyraid Activated? On That User?:/`")
        return
    await Groot.edit("`Please, Wait Fectching User details!`")
    chat_dict = await iter_chats(client)
    chat_len = len(chat_dict)
    if not chat_dict:
        Groot.edit("`You Have No Chats! So Sad`")
        return
    await Groot.edit("`De-Activating Replyraid Raid....!`")
    await runkaal(userz.id)
    ungbanned = f"**De-activated Replyraid Raid [{userz.first_name}](tg://user?id={userz.id})"
    await Groot.edit(ungbanned)
